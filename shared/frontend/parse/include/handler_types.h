...
using PrefixExprHandler = std::function<typename BuilderT::Expr(
    ParseContext<BuilderT> &, frontend::lex::Token)>;

// For ops btw expressions (+, *, <, etc)
template <typename BuilderT>
using InfixExprHandler = std::function<typename BuilderT::Expr(
    ParseContext<BuilderT> &, typename BuilderT::Expr, frontend::lex::Token)>;

// Statements (if, for, var, etc.)
template <typename BuilderT>
using StmtHandler =
    std::function<typename BuilderT::Stmt(ParseContext<BuilderT> &)>;

// For top-level forms (def, extern, module items, etc.)
template <typename BuilderT>
using ItemHandler =
    std::function<typename BuilderT::Item(ParseContext<BuilderT> &)>;
